...
namespace UnityCliMacros
{
    ...
    public static class M_SceneDigest
    {
        ...
        public static string SceneDigest(
            [CliArg("depth", "Tree depth. 0 means auto: 3 normally, whole scene when a filter is set.")]
            int depth = 0,
            [CliArg("root", "Start at one object instead of the scene roots, e.g. /GrayboxMap")]
            string root = "",
            [CliArg("scene", "Open scene by name or path. Defaults to the active scene.")]
            string scene = "",
            [CliArg("filter", "Substring matched against object names AND component type names. Returns a flat list of hierarchy paths instead of an outline.")]
            string filter = "",
            [CliArg("collapse", "Collapse runs of N or more identical siblings. 0 disables collapsing.")]
            int collapse = 3,
            [CliArg("cap", "Maximum objects to walk before truncating")]
            int cap = 4000
        )
        {
        ...
        private static Node Build(
            Transform transform, int depth, int maxDepth, int cap, string parentPath,
        ...
        private static void EmitOne(
            Node node, string title, int collapse, List<string> lines,
        ...
        private static string FormatOutline(
            Scene scene, int rootCount, int totalTransforms,
