...
namespace UnityCliMacros
{
    ...
    public static class M_Inspect
    {
        ...
        /// Distinct prefab property-modification entries reported. A scene-dressed
        /// level instance can carry many hundreds; past this point the list stops
        /// telling you anything new and only costs tokens.
        private const int ModsCap = 40;
        ...
        private const string LEGEND =
            ...
            "rem removed components, modCount total property modifications, modDistinct distinct " +
            "[owner,property] pairs, modElided distinct pairs not listed, " +
            "mods overridden [owner type,property path] with 'xN' when that pair repeats across N objects} " +
            "- absent when the object is " +
        ...
        public static string Inspect(
            [CliArg("target", "The GameObject to inspect. Accepts a hierarchyPath, instanceId, guid or globalId; a Component handle resolves to its owner.", Required = true)]
            ObjectRef target,
            [CliArg("type", "Only report components whose type name contains this substring")]
            string type = "",
            [CliArg("refs", "Report object-reference properties")]
            bool refs = true,
            [CliArg("values", "Also report non-reference serialized values, which is a lot more output")]
            bool values = false,
            [CliArg("children", "Also inspect every descendant, not just this object")]
            bool children = false,
            [CliArg("max", "Maximum property entries reported before truncating")]
            int max = 120
        )
        {
        ...
        private static string RenderObject(
            GameObject gameObject, string type, bool refs, bool values,
            Budget budget
        )
        {
        ...
        private static void AppendProperties(
            StringBuilder sb, Component component, bool refs, bool values,
            Budget budget
        )
        {
        ...
        private static void AppendPrefab(StringBuilder sb, GameObject gameObject)
        {
            ...
            {
                // Collapse identical [owner, propertyPath] pairs and cap the list.
                // A dressed-in-scene level instance routinely carries hundreds of
                // modifications that are the same handful of properties repeated
                // once per descendant (m_StaticEditorFlags across 300 children is
                // the usual offender), and emitting each one buried the useful
                // fields under thousands of tokens of duplicates.
                ...
                foreach (var modification in modifications)
                {
                    ...
                    var key = owner + " " + modification.propertyPath;
                    ...
                    counts[key] = 1;
                    order.Add(new KeyValuePair<string, string>(owner, modification.propertyPath));
                }
                ...
                for (var i = 0; i < order.Count; i++)
                {
                    if (i >= ModsCap)
                    {
                        elided = order.Count - ModsCap;
                        break;
                    }

                    var entry = order[i];
                    var count = counts[entry.Key + " " + entry.Value];
                    var label = count > 1 ? entry.Value + " x" + count : entry.Value;
                    paths.Add(new KeyValuePair<string, string>(entry.Key, label));
                }

                sb.Append(",\"modCount\":").Append(modifications.Length);
                sb.Append(",\"modDistinct\":").Append(order.Count);

                if (elided > 0)
                {
                    sb.Append(",\"modElided\":").Append(elided);
                }
